...
const deleteNote = async (req, res) => {
    try {
        ...
        const noteDelete = await Note.findOneAndDelete({ _id: req.params.id, user_id: req.user._id })
        if (!noteDelete) {
...
const getNoteDetailsById = async (req, res) => {
    try {
        ...
        const noteDetails = await Note.findOne({ _id: req.params.id, user_id: req.user._id });
...
const fetchAllNotes = async (req, res) => {
    try {
        ...
        allNotes = await Note.find({user_id: req.user._id}).sort({createdAt: -1});
...
const updateNotesDetailsById = async (req, res) => {
    try {
        ...
        const noteUpdate = await Note.findByIdAndUpdate({_id: req.body._id, user_id: req.user._id}, req.body, {
            new: true
        })
...
const updateNoteStatus = async (req, res) => {
    try {
        ...
        let noteStatusUpdate = await Note.findByIdAndUpdate({_id: req.body._id, user_id: req.user._id}, {note_status:req.body.note_status}, {
            new: true
        })
